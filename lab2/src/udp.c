 ...
 * @param  dest: la dirección a la que se quiere enviar el mensaje
 * @param  data: el mensaje a enviar
 * @param  len: la longitud del mensaje
 *
 * @retval Ninguno
 */
void udp_write(struct udp_socket_t* conn, struct sockaddr_in* dest,
               const char* data, size_t len) {
    sendto(conn->fd, data, len, 0, (struct sockaddr*)dest, sizeof(*dest));
}
 ...
 *		del remitente (opcional). Si no se va a utilizar, reemplazar
 *		por `NULL`
