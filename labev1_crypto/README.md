...
En esta carpeta encontrarán un proyecto con código de referencia para el cifrado y descifrado simétrico de datos con `libsodium` usando una llave secreta. El código que actúa de _wrapper_ para `libsodium` lo pueden encontrar en `src/crypto.c` e `include/crypto.h`, donde verán implementados un conjunto de funciones que les puede facilitar el uso de esta biblioteca.

## Instalación y compilación

En sistemas tipo Debian (como Ubuntu y Linux Mint), necesitan instalar el paquete `libsodium-dev` para poder compilar este código. En sistemas basados en Arch ~btw~, encontrarán la biblioteca en `libsodium`.
